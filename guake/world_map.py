...
class WorldMapView(Gtk.ScrolledWindow):
    ...
    def __init__(self, guake_app):
        ...
        # Caching and Refresh Timer
        self._git_status_cache = {}
        self._all_terminals_map = {}
        self._refresh_timer_id = None
        self.connect("show", self._on_view_shown)
        self.connect("hide", self._on_view_hidden)
        
        notebook = self.guake_app.get_notebook()
        notebook.connect("page-added", self._on_tab_structure_changed)
        notebook.connect("page-removed", self._on_tab_structure_changed)
    ...
    def _on_view_shown(self, widget):
        """Called when the world map becomes visible."""
        log.debug("World Map shown, starting refresh timer and forcing data update.")
        self._start_refresh_timer()
        self.refresh_view(force_data_update=True)

    def _on_view_hidden(self, widget):
        """Called when the world map is hidden."""
        log.debug("World Map hidden, stopping refresh timer.")
        self._stop_refresh_timer()

    def _on_tab_structure_changed(self, notebook, page, page_num):
        """Handler for when tabs are added or removed."""
        log.debug("Tab structure changed, forcing a data refresh.")
        if self.get_visible():
            self.refresh_view(force_data_update=True)
    ...
    def _start_refresh_timer(self):
        ...
        if self._refresh_timer_id is None:
            log.info("Starting world map refresh timer (15s).")
    ...
    def _stop_refresh_timer(self):
        ...
        if self._refresh_timer_id is not None:
            log.info("Stopping world map refresh timer.")
            GLib.source_remove(self._refresh_timer_id)
            self._refresh_timer_id = None

    def _timed_refresh(self):
        """The callback for the GLib timer to periodically refresh data."""
        log.debug("Timed refresh triggered.")
        self.refresh_view(force_data_update=True)
        return True # Keep the timer going

    def _update_data_cache(self):
        """Updates the internal cache of terminal data. This is the slow part."""
        log.debug("Updating data cache (terminals and Git status).")
        self._all_terminals_map.clear()
        notebook = self.guake_app.get_notebook()
        for i in range(notebook.get_n_pages()):
             page = notebook.get_nth_page(i)
             for term in page.iter_terminals():
                 self._all_terminals_map[str(term.uuid)] = (term, i)

        self._git_status_cache.clear()
        git_counts = {'clean': 0, 'dirty': 0, 'untracked': 0, 'no-git': 0}
        for uuid, (term, _) in self._all_terminals_map.items():
            try:
                cwd = term.get_current_directory()
                status = self._get_git_status(cwd)
                self._git_status_cache[uuid] = status
                if status in git_counts:
                    git_counts[status] += 1
            except Exception:
                self._git_status_cache[uuid] = 'no-git'
                git_counts['no-git'] += 1
        ...
        self.layout.synchronize(self._all_terminals_map)

    def refresh_view(self, force_data_update=False):
        """Refreshes the entire world map view. Can optionally force a slow data update."""
        ...
        self.toggle_all_button.set_label("Collapse All" if is_any_expanded else "Expand All")
        ...
        for project in visible_projects:
            ...
            flowbox = Gtk.FlowBox(valign=Gtk.Align.START, selection_mode=Gtk.SelectionMode.NONE, column_spacing=15, row_spacing=15, margin_top=10)
            ...
            if is_expanded:
                ...
                self._redraw_project_grid(project, self._all_terminals_map, self._git_status_cache)
            else:
    ...
    def _redraw_project_grid(self, project, all_terminals_map, git_status_map):
        ...
        for term_uuid in project.get("terminals", []):
            if term_uuid in all_terminals_map:
                ...
                git_status = git_status_map.get(term_uuid, 'no-git')
                preview_widget = self._create_terminal_preview(terminal, page_num, git_status)
    ...
    def _summarize_path(self, path):
        ...
        if len(parts) > 4: path = os.sep.join([parts[0], '...', parts[-2], parts[-1]])
        return path

    def _create_terminal_preview(self, terminal, page_num, git_status):
        notebook = self.guake_app.get_notebook()
        ...
        try: cwd = terminal.get_current_directory()
        except Exception: cwd = None

        if git_status == 'clean': icon_name, tooltip = "emblem-ok-symbolic", "Git: Clean"
        elif git_status == 'dirty': icon_name, tooltip = "emblem-synchronizing-symbolic", "Git: Uncommitted changes"
        elif git_status == 'untracked': icon_name, tooltip = "document-new-symbolic", "Git: Untracked files"
        else: icon_name, tooltip = "emblem-important-symbolic", "Not a Git repository"
        
        git_icon = Gtk.Image.new_from_icon_name(icon_name, Gtk.IconSize.MENU)
        git_icon.set_tooltip_text(tooltip)
        git_icon.get_style_context().add_class(f"git-status-{git_status}")
        ...
        path_label.set_markup(f"<small>{self._summarize_path(cwd)}</small>")
    ...
    def move_terminal_to_project(self, terminal_uuid, target_project_title):
        ...
        self._move_terminal_in_layout(target_uuid, target_project)
